...
This scraper extracts job listings from YARDI CALL CENTER SRL careers page (Romania only) and imports them to peviitor.ro.

Target: https://yardiromania.breezy.hr
...
1. **Start with brand** - We know the brand (e.g., "YARDI")
...
8. **Scrape new jobs** - Extract jobs from yardiromania.breezy.hr (Romania)
...
> **Important**: Scraper does NOT delete jobs from other sources (ANOFM, etc). It only upserts Yardi Careers jobs. Existing jobs are preserved.
...
3. **Scrape jobs** - Extract jobs from Yardi Breezy API (Romania only)
...
company.js (validate company)
    ...
    │
    ▼ (if active)
scrape Yardi Breezy API (jobs for Romania)
    │
    ▼
...
| `tests/validate-yardi-jobs.js` | CI fast validator (HEAD only); thin CLI over `src/job-validator.js` + `solr.js` |
...
| `tests/e2e/scraper.test.js` | End-to-end tests with real Yardi Breezy API |
