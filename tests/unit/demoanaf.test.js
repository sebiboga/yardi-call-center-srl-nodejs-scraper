...
describe('src/anaf.js', () => {
  ...
  describe('searchCompany', () => {
    it('should return array of companies for valid brand', async () => {
      mockFetch.mockResolvedValue(anafSearchResponse([
        { cui: 32509291, name: 'YARDI CALL CENTER SRL', statusLabel: 'Funcțiune' }
      ]));

      const results = await anaf.searchCompany('YARDI');
    ...
    it('should throw on HTTP error', async () => {
      ...
      await expect(anaf.searchCompany('YARDI')).rejects.toThrow('ANAF search error: 500');
    });
    ...
    it('should encode brand name in URL', async () => {
      ...
      await anaf.searchCompany('YARDI CALL CENTER');
      expect(capturedUrl).toContain(encodeURIComponent('YARDI CALL CENTER'));
    });
  });
  ...
  describe('getCompanyFromANAF', () => {
    it('should return company data for valid CIF', async () => {
      ...
      const data = await anaf.getCompanyFromANAF('32509291');

      expect(data).toBeDefined();
      expect(data.cui).toBe(32509291);
      expect(data.name).toBe('YARDI CALL CENTER SRL');
    ...
    it('should throw after exhausting retries', async () => {
      ...
      await expect(anaf.getCompanyFromANAF('32509291')).rejects.toThrow();
  ...
  describe('getCompanyFromANAFWithFallback', () => {
    it('should return fresh data when API works', async () => {
      ...
      const data = await anaf.getCompanyFromANAFWithFallback('32509291');
    ...
    it('should use cached data when API fails', async () => {
      ...
      const data = await anaf.getCompanyFromANAFWithFallback('32509291', CACHED_DATA);
    ...
    it('should throw when API fails and no cache available', async () => {
      ...
      await expect(anaf.getCompanyFromANAFWithFallback('32509291')).rejects.toThrow();
    });
  });
});
